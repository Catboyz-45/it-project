...
export function OwnerGlobalSearch({ propertyId }: Readonly<{ propertyId: string }>) {
  ...
  return <div className="relative min-w-[260px]">
    <label className="relative block">
      <span className="sr-only">ค้นหาทั้งหอ</span>
      <Search className="absolute top-1/2 left-3 -translate-y-1/2 text-[#73757d]" size={17} />
      <input className="w-full pl-10 pr-12" onChange={(e) => setQuery(e.target.value)} placeholder="ค้นหาผู้เช่า ห้อง บิล สัญญา" value={query} />
      {query ? <IconButton className="absolute top-1/2 right-0 -translate-y-1/2" label="ล้างคำค้น" onClick={() => setQuery("")}><X size={16} /></IconButton> : null}
    </label>
    {/* ผลลัพธ์ลอยทับเนื้อหาข้างล่าง และซ่อนไปเมื่อคำค้นสั้นกว่า 2 ตัว */}
    {query.trim().length >= 2 ? <div className="absolute right-0 z-50 mt-2 max-h-80 w-full min-w-[340px] overflow-auto rounded-2xl border border-white/10 bg-[#171821] p-2 text-white shadow-2xl">
      <SearchResults loading={loading} onSelect={() => setQuery("")} results={results} />
    </div> : null}
  </div>;
}

// เนื้อในกล่องผลลัพธ์ กำลังค้น เจอ หรือไม่เจอ
function SearchResults({ loading, onSelect, results }: Readonly<{
  loading: boolean;
  onSelect: () => void;
  results: Result[];
}>) {
  if (loading) return <p className="p-3 text-sm text-white/50">กำลังค้นหา...</p>;
  if (results.length === 0) {
    return <output className="block rounded-xl bg-red-500/10 p-3 text-center text-sm font-bold text-red-300">ไม่พบข้อมูลที่ค้นหา</output>;
  }
  return <>{results.map((item) => <Link className="block rounded-xl p-3 hover:bg-white/10" href={item.href} key={`${item.type}-${item.id}`} onClick={onSelect}>
    <small className="text-brand-green">{labels[item.type]}</small>
